...
public class PropagatingTracingMessageUtils {
	...
	public static SpanContext extract(Message message, Tracer tracer) {
		SpanContext spanContext = tracer.extract(Format.Builtin.TEXT_MAP, new JmsTextMapExtractAdapter(message));
		if (spanContext != null) {
			return spanContext;
		}
	...
	public static Span buildAndInjectSpan(Destination destination, final Message message, Tracer tracer)
			throws JMSException {
		Tracer.SpanBuilder spanBuilder = tracer.buildSpan(TracingMessageUtils.OPERATION_NAME_SEND).ignoreActiveSpan()
